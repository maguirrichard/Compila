...
public class AnalisadorLexico{
    ...
    public RegistroLexico automato(boolean concluido, char char_atual) throws IOException{
        ...
        while(estadoAtual != estadoFinal){
            switch(estadoAtual){
                ...
                case 13: //id começando com letra
                    ...
                    break;
                default:
                    System.out.println(num_linha + "caractere invalido " + char_atual); //o caractere encontrado não corresponde a nenhum possível
                    break;
            }
        }
        if(fim){
        ...
        else if(!(digito(linha.charAt(0)) && ((int) linha.charAt(0)) == 39)){ //verifica se o lexema é um id ou palavra reservada
            ...
            else if((int) linha.charAt(0) != 39 && ((int) (linha.length() - 1)) != 39 ) { //não é uma string, então não é uma constante
                constante = false;
            }
            else {
                tipo = "string";
